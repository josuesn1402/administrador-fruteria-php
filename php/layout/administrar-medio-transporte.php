...
$queryTransporte = "SELECT medio_id, tipo, capacidad FROM medio_transporte";
    ...
    <form id="transporteForm" method="POST" action="../controllers/medioTransporteCtrl/registrarCtrl.php" class="grid gap-6">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="flex flex-col">
          ...
          <input type="text" id="capacidad" name="capacidad" class="p-2 border rounded-lg">
        </div>
      </div>
            ...
            <tbody>
              ...
              if ($resultTransporte->num_rows > 0) {
                while ($row = $resultTransporte->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td class=\"px-4 py-2 border\">" . $row['medio_id'] . "</td>";
                  echo "<td class=\"px-4 py-2 border\">" . $row['tipo'] . "</td>";
                  echo "<td class=\"px-4 py-2 border\">" . $row['capacidad'] . "</td>";
                  echo "<td class=\"px-4 py-2 border\"><a class='btn-editar' href='modificar-medio-transporte.php?id=" . $row['medio_id'] . "'><img src=\"../../assets/svg/edit.svg\" class=\"inline h-5\" alt=\"\"></a> <a class='btn-eliminar' href='../controllers/medioTransporteCtrl/eliminarCtrl.php?id=" . $row['medio_id'] . "' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'><img src=\"../../assets/svg/delete.svg\" class=\"inline h-5\" alt=\"\"></a></a></td>";
                  echo "</tr>";
                }
              } else {
